 ...
 *
 * NOTE: if it suspended for a long time, this might actually be
 * an event-driven new job release.
 *
 */
static void demo_task_resume(struct task_struct  *tsk)
{
	...
	if (is_tardy(tsk, now)) {
		/* This task was gone for a "long" time and woke up past
			  ...
			  int is_running)
{
	...
	/* the first job exists starting of right now */
	release_at(tsk, now);
